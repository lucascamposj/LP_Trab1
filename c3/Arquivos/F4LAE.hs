...
interp (LambdaApp e1 e2) subs decs = interp e ((form_arg,expression) : subs_closure) decs -- Terminada ( falta testar ) decs_closure= lista closure
  where
    expression = interp e2 subs decs
    (Closure form_arg e subs_closure) = interp e1 subs decs
